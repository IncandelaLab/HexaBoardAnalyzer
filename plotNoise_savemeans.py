...
def getChansData(tree, chip = 0, chans = [0], sca = 0, variabs = []):
    ...
    for ientry, entry in enumerate(tree):
        ...
        #this gives option to stop after certain # of events for quick analysis
        stopatevent=1000;
        if ientry ==stopatevent:
            break

        # filter sca by time sample (before trigger) this filters rollmask==1!
    ...
    for var in variabs:
        ...
        if 'hg' in var:
            ...
            for chan in sorted(rms_data.keys(), key = lambda x:rms_data[x][1], reverse=True):
                ...
                fout.write("%i %i\t%0.2f\t%i\n" %(chan/64,chan%64,rms_data[chan][1],int(rms_data[chan][0])))
