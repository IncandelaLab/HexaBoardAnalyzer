...
def read_root_file(filename):
    ...
    for sample in range(13):
        lgsam=[];hgsam=[]
        treedata=readTree(filename, chip='all',sca=sample,chan_select='even')#sca is sample#
        chans = treedata.keys()
...
#plots histogram of the rms for each channel (all events and samples) in a pcb if plot is true and returns the median rms over all channels in that board
    ...
    chan_rms=np.array(chan_rms)
    chan_rms=chan_rms[[np.isfinite(chan_rms)]] #this is to get rid of nan's if only even/odd channels where loaded
    ...
    if plot:
        ...
        #plt.figure()
        ax.hist(chan_rms, 20, normed=0, facecolor=color,alpha=0.7)
        ax.axvline(medrms, color='k', linestyle='dashed', linewidth=2)
        plt.xlabel('RMS per channel', fontsize=39)
	plt.ylabel('# of channels', fontsize=40)
        
        #plt.show()
	#if gain=='HG'or gain=='hg': plt.xlim(4,13)
	ax.tick_params(labelsize=40)
        
	print "median=%.2f\nmean=%.2f\nrms=%.2f" % (medrms,np.mean(chan_rms),np.std(chan_rms))
        statStr=("median=%.2f\nmean=%.2f\nrms=%.2f" % (medrms,np.mean(chan_rms),np.std(chan_rms)))
	props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
	ax.text(0.95, 0.95, statStr,  transform=ax.transAxes, fontsize=34,verticalalignment='top', horizontalalignment='right',bbox=props)	
	fig.savefig(Dir+gain+'board_rms_dist'+savetype)       
	#plt.show()
    if returnarr:
